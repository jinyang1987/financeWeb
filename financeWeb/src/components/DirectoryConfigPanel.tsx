...
const Checkbox = ({ checked, onCheckedChange, className = '' }: { 
  ...
  <input
    ...
    className={`w-4 h-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500 cursor-pointer ${className}`}
  />
);
...
// 面板展示完整层级用于勾选，三个视图按 ARCHIVE_ITEM_MAP 层级展示
  ...
  Object.entries(ARCHIVE_CONTENT_CONFIG).forEach(([category, config]) => {
    if (config.children) {
      Object.entries(config.children).forEach(([subCategory, subConfig]) => {
        if ('items' in subConfig && subConfig.items) items.push(...(subConfig.items as string[]));
        else if ('children' in subConfig && subConfig.children) {
          Object.entries(subConfig.children as Record<string, { items: string[] }>).forEach(([thirdCategory, thirdConfig]) => {
...
export default function DirectoryConfigPanel() {
  ...
  // 初始化
  useEffect(() => {
    if (loading) return; // 等配置从 API 加载完成再初始化（否则拿默认值覆盖）
    // ★ 回读已保存的勾选（P0-9 修复：原先恒为全选，保存即覆盖用户此前的勾选）
    const saved = config.selectedArchiveItems;
    if (Array.isArray(saved) && saved.length > 0) {
      setSelectedArchiveItems(new Set(saved));
    } else {
      setSelectedArchiveItems(new Set(getAllArchiveItems()));
    }
  ...
  // 档案配置子树节点类型
  interface ArchiveContentSubNode {
    items?: string[];
    children?: Record<string, ArchiveContentSubNode>;
  }

  /** 从子树节点或子节点映射中递归收集所有 items */
  const getSubArchiveItems = (subConfig: ArchiveContentSubNode | Record<string, ArchiveContentSubNode>): string[] => {
    // 如果传入的是子节点映射（如 catConfig.children），遍历所有子节点
    if (!('items' in subConfig) && !('children' in subConfig)) {
      const items: string[] = [];
      Object.values(subConfig).forEach((child: ArchiveContentSubNode) => {
        if (child.items) items.push(...child.items);
        if (child.children) {
          Object.values(child.children).forEach((grandChild: ArchiveContentSubNode) => {
            if (grandChild.items) items.push(...grandChild.items);
          });
        }
      });
      return items;
    ...
    // 传入的是单个子树节点
    if ('items' in subConfig && subConfig.items) return subConfig.items as string[];
    if ('children' in subConfig && subConfig.children) {
      const items: string[] = [];
      Object.values(subConfig.children).forEach((child: ArchiveContentSubNode) => {
        if (child.items) items.push(...child.items);
      });
      return items;
    }
  ...
  // 页签配置
  const tabs = [
    { id: 'archive-category', label: '选择类型', icon: Archive },
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-t-lg transition-all text-sm font-medium ${
                  isActive
                    ? 'bg-slate-100 text-slate-800 border-b-2 border-sky-600'
        ...
        {activeTab === 'archive-category' && (
          <div className="p-6">
            <div className="mb-4 text-sm text-slate-500">
              已选 <span className="font-bold text-sky-600">{selectedArchiveItems.size}</span> / {TOTAL_ARCHIVE_ITEMS} 项档案类型
            </div>
              ...
              {Object.entries(ARCHIVE_CONTENT_CONFIG).map(([category, catConfig]) => {
                ...
                return (
                  <div key={category} className={`rounded-xl border ${catConfig.bgColor} border-slate-200`}>
                    ...
                    {isExpanded && catConfig.children && Object.keys(catConfig.children).length > 0 && (
                            ...
                            <div key={subCategory} className="ml-3">
                              ...
                              {'items' in subConfig && subConfig.items ? (
                                ...
                                <div className="ml-5 space-y-1.5">
                                  {Object.entries(subConfig.children as Record<string, { items: string[] }>).map(([thirdCategory, thirdConfig]) => {
                ...
                return (
                  <button
                    ...
                    className={`relative p-3 rounded-xl border-2 transition-all cursor-pointer ${
                      isSelected ? 'bg-sky-50 border-sky-400 shadow-sm' : 'bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50'
                    }`}
                  >
                        ...
                        {editingProject === project.id ? (
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className="px-2 py-1 border border-slate-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                            />
                            <input
                              type="text"
                              value={project.code}
                              onChange={(e) => updateProject(project.id, 'code', e.target.value)}
                              className="w-20 px-2 py-1 border border-slate-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                            />
                            ...
                            return (
                              <div key={stage} className="bg-white rounded-lg border border-slate-200">
                                ...
                                {isStageExpanded && stageConfig.phases && (
                                  <div className="px-3 pb-3 space-y-3">
                                    {Object.entries(stageConfig.phases as Record<string, { desc?: string; items: string[] }>).map(([phaseName, phaseConfig]) => {
