            ...
            {childRecords.map((child) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {child.remarks && (
                    <div className="text-xs text-sky-400 mt-0.5 truncate">{child.remarks}</div>
                  )}
                </div>
        ...
        {richSourceDocs.length > 0 && (
          <div className="space-y-1.5">
            ...
            {richSourceDocs.map((sd) => (
              ...
              >
                <div className="w-1.5 h-1.5 rounded-full whitespace-nowrap bg-emerald-400 shrink-0 mt-1.5" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-semibold text-emerald-800">{sd.docTypeName}</span>
                    <span className="text-xs text-emerald-500 font-mono">{sd.documentNo}</span>
                  </div>
                  <div className="text-xs text-emerald-600 mt-0.5">
                    {sd.counterpartyName} · ¥{Math.abs(sd.amountLower).toLocaleString()}
                  </div>
                  <div className="text-xs text-emerald-400 mt-0.5 line-clamp-1">{sd.summary}</div>
                </div>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {activeRecord.carrierType === 'electronic' || activeRecord.source === 'digital-native' ? (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full whitespace-nowrap text-xs font-bold bg-sky-50 text-sky-700 border border-sky-200">
                    <Monitor className="w-3 h-3" /> 纯电子文件
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full whitespace-nowrap text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200">
          ...
          <span className="px-2 py-0.5 rounded-full whitespace-nowrap bg-slate-100 text-xs font-mono font-bold text-slate-500">
            ID: {activeRecord.id}
          </span>
          <button
            type="button"
            onClick={closeDrawer}
            className="p-1.5 hover:bg-slate-100 rounded-full whitespace-nowrap text-slate-400 hover:text-slate-800 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            return (
              <div className="bg-white border border-slate-200 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full whitespace-nowrap">{entries.length} 条</span>
                </div>
                <DetailRows
          ...
          {(activeRecord.carrierType === 'paper' || activeRecord.source === 'digitized') && (
            <div className="bg-amber-50/50 border border-amber-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-amber-500 bg-amber-100 px-1.5 py-0.5 rounded-full whitespace-nowrap">按卷管理</span>
              </div>
              <FieldGrid
                columns={2}
                fields={[
