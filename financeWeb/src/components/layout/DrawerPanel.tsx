    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {activeRecord.carrierType === 'electronic' || activeRecord.source === 'digital-native' ? (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs font-bold bg-sky-50 text-sky-700 border border-sky-200">
                    <Monitor className="w-3 h-3" /> 纯电子文件
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200">
          ...
          <span className="px-2 py-0.5 rounded-full bg-slate-100 text-xs font-mono font-bold text-slate-500">
            ID: {activeRecord.id}
          </span>
          <button
            type="button"
            ...
            return (
              <div className="bg-white border border-slate-200 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full">{entries.length} 条</span>
                </div>
                <DetailRows
          ...
          {(activeRecord.carrierType === 'paper' || activeRecord.source === 'digitized') && (
            <div className="bg-amber-50/50 border border-amber-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-amber-500 bg-amber-100 px-1.5 py-0.5 rounded-full">按卷管理</span>
              </div>
              <FieldGrid
                columns={2}
                fields={[
