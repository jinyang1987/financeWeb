...
import { SecurityWatermark } from './watermark';
...
export const InteractivePreview: React.FC<InteractivePreviewProps> = ({
  ...
  const handleDownload = () => {
    if (watermarkConfig.enabled && watermarkConfig.scenes.download) {
      alert(
        `组件「${activeFile?.name}」已加入下载队列。\n\n` +
        `按水印策略，后端服务将把溯源水印烧录进文件本体后返回文件流` +
        `${watermarkConfig.download.asyncQueue ? '（异步排队）' : ''}。` +
        `${watermarkConfig.download.failSecure ? '\n加水印服务异常时将阻断下载（安全失败原则）。' : ''}`
      );
    } else {
      alert(`组件: ${activeFile?.name} 的原始哈希验签文件已载入下载队列`);
    }
  };

  // 打印：水印策略启用时提示带水印打印流（需求第2.3节）
  const handlePrint = () => {
    if (watermarkConfig.enabled && watermarkConfig.scenes.print) {
      alert('已调用带水印的预览流进行打印（浏览器原生无水印打印已屏蔽）');
    } else {
      alert('已调用本地打印模组发出脱机排版PDF');
    }
  };
        ...
        <div id="document-preview-frame" className="border border-slate-200 bg-slate-50/50 rounded-2xl flex-1 flex flex-col overflow-hidden relative min-h-[300px]">
          {activeFile ? (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="flex-1 p-5 overflow-y-auto bg-white flex flex-col justify-start relative">
                ...
                {activeFile.contentType === 'xml' && (
                  <div className="border-2 border-slate-100 rounded-xl p-4 max-w-2xl mx-auto w-full font-mono text-[11px] space-y-1 bg-slate-50 leading-relaxed shadow-sm">
                    ...
                    <div className="pl-4">
                      <p className="text-sky-600">&lt;InvoiceHeader&gt;</p>
                      <div className="pl-4">
                        ...
                        <p className="text-teal-600">&lt;SettleAmount&gt;<span className="text-sky-700 font-bold">{record.amount}</span>&lt;/SettleAmount&gt;</p>
                        <p className="text-teal-600">&lt;TaxAmount&gt;<span className="text-slate-800">{(record.amount * 0.06).toFixed(2)}</span>&lt;/TaxAmount&gt;</p>
                        <p className="text-teal-600">&lt;InvoiceDate&gt;<span className="text-slate-800">2026-05-30</span>&lt;/InvoiceDate&gt;</p>
                      </div>
                      <p className="text-sky-600">&lt;/InvoiceHeader&gt;</p>
                      <p className="text-sky-600">&lt;DigitalSignatureVerified&gt;</p>
                      <div className="pl-4">
                    ...
                    <p className="text-sky-600">&lt;/InvoiceRoot&gt;</p>
                  </div>
                )}
                ...
                {activeFile.contentType === 'ofd' && (
                  <div className="flex-1 flex flex-col justify-center items-center py-6">
                    ...
                    {!record.checks.usable ? (
                      ...
                      <div className="border border-sky-200 bg-white rounded-xl p-5 max-w-lg w-full shadow-sm relative overflow-hidden" id="ofd-success-mock">
                        
                        {/* Legal watermark stamp */}
                        <div className="absolute top-10 right-10 border-2 border-sky-500 font-bold px-3 py-1 text-xs text-sky-500 rounded uppercase transform rotate-[-12deg] tracking-wide pointer-events-none select-none select-none">
                          长期凭证 OFD/A
                        </div>

                        <div className="border-b border-sky-100 pb-3 mb-4 flex items-center gap-2">
                        ...
                        <div className="space-y-2.5 text-xs text-slate-600">
                          <div className="flex justify-between py-1 border-b border-slate-50">
                            <span>原始主体编码:</span>
                            <span className="font-mono text-slate-800">CN-XFZ-92427ae</span>
                          </div>
                          <div className="flex justify-between py-1 border-b border-slate-50">
                            <span>业务代表签署人:</span>
                            <span className="font-sans text-slate-800">{activeFile.signer || '系统核心授权印签'}</span>
                          </div>
                          <div className="flex justify-between py-1 border-b border-slate-50">
                            <span>所属财务公宗 (Fonds Code):</span>
                            <span className="font-mono text-slate-800">Z001 (华北集团总部)</span>
                          </div>
                        </div>
