  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
        <h2 className="text-base font-bold text-slate-900 border-l-4 border-sky-600 pl-3 mb-2 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 bg-slate-50 border border-slate-200 rounded-xl p-5 flex flex-col justify-between">
                  ...
                  <div className="inline-flex rounded-lg border border-slate-300 p-0.5 bg-white">
                    {[1, 2, 3].map(rowNo => (
                      <button
                        ...
                        className={`px-3 py-1 font-bold rounded-lg transition-all text-xs cursor-pointer ${activeRow === rowNo ? 'bg-sky-600 text-white' : 'text-slate-500 hover:text-slate-800'}`}
                      >
                        第 {rowNo} 排密集架
                      </button>
                    ))}
                    ...
                    <div
                      ...
                      className={`relative aspect-square border rounded-xl p-3 flex flex-col justify-between cursor-pointer transition-all ${
                        slot.status === 'occupied' 
                          ? isSelected ? 'bg-emerald-100/90 border-sky-600 ring-2 ring-sky-500/20 shadow-md' : 'bg-white border-emerald-300 hover:border-emerald-500 hover:shadow-xs'
                          : isSelected ? 'bg-rose-100/90 border-sky-600 ring-2 ring-sky-500/20 shadow-md' : 'bg-white border-dashed border-rose-200 hover:border-rose-400 hover:shadow-xs'
                      }`}
                    >
                      ...
                      {slot.status === 'occupied' ? (
                        <div className="space-y-0.5 mt-2">
                          <span className="block font-bold text-[11px] text-slate-800 truncate">{slot.category}</span>
                      ...
                      <span className="absolute bottom-1 right-2 text-[9px] font-mono text-slate-300 select-none">R{slot.row}-C{slot.col}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="mt-5 p-3.5 bg-sky-50 border border-sky-100/80 rounded-xl text-[11.5px] text-sky-950 flex items-center gap-2.5">
              <Layers className="w-4 h-4 text-sky-500 text-sky-600 shrink-0" />
          ...
          <div className="lg:col-span-4 bg-slate-50 border border-slate-200 rounded-xl p-5 flex flex-col justify-between">
            <div>
              ...
              {selectedSlot ? (
                <div className="space-y-4 text-xs">
                  ...
                  <div className="divide-y divide-slate-100 border-y border-slate-100 py-1.5 space-y-1.5">
                    <div className="pt-1 flex justify-between">
                      <span className="text-slate-400 text-slate-400">目前状态:</span>
                    ...
                    {selectedSlot.status === 'occupied' && (
                      <>
                        ...
                        <div className="pt-1.5 flex justify-between font-serif">
                          ...
                          <span className="font-bold text-sky-600">{selectedSlot.attachmentCount} 张电子凭片</span>
                        </div>
            ...
            {selectedSlot && (
              <button
                ...
                className="w-full mt-4 py-2 bg-sky-600 hover:bg-sky-700 bg-sky-600 text-white font-bold text-xs rounded-lg shadow-xs cursor-pointer transition-colors"
              >
                保存本物理架仓位属性
              </button>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="bg-slate-50 bg-slate-50 border border-slate-100 p-4 rounded-xl space-y-4 mb-5">
              ...
              {hsmStep === 'running' && (
                <div className="space-y-1.5 animate-in fade-in">
                  <div className="flex justify-between text-[10px] font-mono font-bold text-sky-600">
                    <span>迁移进度 ({hsmProgress}%) ...</span>
                    <span>{hsmProgress < 60 ? '正在加密封包...' : '正在同步写物理光盘...'}</span>
                  </div>
                  <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-sky-500 to-sky-600 rounded-full transition-all duration-300" style={{ width: `${hsmProgress}%` }} />
                  </div>
                </div>
              )}

              {hsmStep === 'success' && (
                <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-lg flex items-center gap-2 text-[11px] text-emerald-800 font-medium animate-in zoom-in-95 duration-200">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs font-sans">
                <div className="space-y-1">
                  <span className="block text-[10px] text-slate-400 text-slate-400">生命周期流速阀门</span>
                  <select
                ...
                <div className="space-y-1">
                  <span className="block text-[10px] text-slate-400 text-slate-400">平移归藏物理网</span>
                  <select
            ...
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-3.5 mb-4">
                ...
                <>
                  <div className="flex justify-between items-center text-[10.5px] border-b border-slate-200 pb-1.5">
                    <span className="font-bold text-rose-800 bg-rose-50 px-2 py-0.5 rounded-md flex items-center gap-1">
                      <ShieldAlert className="w-3.5 h-3.5 text-rose-500 text-rose-500" />
                      国家合规到期审计检测
                    </span>
              ...
              {destructionStep === 2 && (
                <div className="space-y-3">
                  <div className="text-center font-bold text-xs text-slate-800 border-b border-slate-100 pb-2 flex items-center justify-center gap-1">
          ...
          <div>
            ...
            {destructionStep === 2 && (
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl font-bold cursor-pointer text-xs"
                >
                  回退清单
                </button>
                <button
            ...
            {destructionStep === 3 && (
              <button
                ...
                className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-sky-600 font-bold text-xs rounded-xl cursor-pointer transition-colors"
              >
                巡检下一批到期鉴定件
              </button>
            )}
