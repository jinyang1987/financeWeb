        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="border-t border-slate-100 pt-3">
            ...
            <div className="space-y-3">
              ...
              <label className="block">
                <span className="text-xs text-slate-500">默认去向</span>
                <select
          ...
          {sources.map((s) => (
            <div key={s.id} className={`bg-white border rounded-xl p-4 ${s.enabled ? 'border-slate-200' : 'border-slate-100 opacity-70'}`}>
              ...
              <div className="mt-3 flex items-center gap-2 flex-wrap">
                <span className="px-2 py-0.5 text-xs font-medium rounded-full whitespace-nowrap bg-sky-50 text-sky-700 border border-sky-200">
                  默认去向：{DEST_LABELS[s.config.defaultDestination] || '送组卷工作台'}
                </span>
                {s.config.scheduleEnabled === 'true' ? (
                  <span className="px-2 py-0.5 text-xs font-medium rounded-full whitespace-nowrap bg-emerald-50 text-emerald-700 border border-emerald-200" title={s.config.scheduleCron}>
                    定时抓取：{s.config.scheduleCron || 'cron 未设置'}
                  </span>
                ) : (
                  <span className="px-2 py-0.5 text-xs rounded-full whitespace-nowrap bg-slate-100 text-slate-500 border border-slate-200">
                    定时抓取：未启用
                  </span>
                )}
              </div>
