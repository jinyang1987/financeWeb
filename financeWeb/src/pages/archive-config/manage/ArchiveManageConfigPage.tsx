...
const ArchiveManageConfigPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100">
      {/* ═══ 顶部 Tab 栏（2026-09-07 防折行硬约束：页签永不折行，窄屏横向滚动兜底） ═══ */}
      <div className="flex items-center gap-4 px-6 py-2.5 bg-white border-b border-slate-200 shrink-0 flex-nowrap overflow-x-auto no-scrollbar min-w-0">
        <div className="flex items-center bg-slate-100 rounded-lg p-0.5 shrink-0">
          {TABS.map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-1.5 text-sm font-medium whitespace-nowrap shrink-0 rounded-md transition-colors cursor-pointer ${
                tab === t.key
