...
const CategoryCard: React.FC<{ category: ArchiveCategory; defaultExpanded?: boolean }> = ({
  category,
...
const DetailRow: React.FC<{ detail: CategoryDetail; isLast: boolean; depth: number }> = ({
  detail,
  isLast,
  depth,
}) => {
      ...
      <div className="flex-1 overflow-y-auto p-6 max-w-5xl mx-auto w-full space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3 px-1">
            ...
            <span className="text-[11px] text-slate-400">共 4 大类 {totalDetailCount} 项明细，选定明细后系统自动填充对应保管期限</span>
          </div>
          ...
          <div className="space-y-3">
            {ARCHIVE_CATEGORIES.map((cat) => (
              <CategoryCard
                key={cat.code}
                category={cat}
                defaultExpanded={expandedCategories[cat.code]}
              />
            ))}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-red-50/50 border-b border-slate-100">
                  <th className="px-5 py-2.5 text-xs font-semibold text-slate-600 text-left">材料类型</th>
                  <th className="px-5 py-2.5 text-xs font-semibold text-slate-600 text-center w-32">归入档案类型</th>
                  <th className="px-5 py-2.5 text-xs font-semibold text-slate-600 text-left">备注说明</th>
                </tr>
              </thead>
              <tbody>
                {EXCLUSION_ITEMS.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-xs text-slate-500">{item.remark}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* ── 页脚 ── */}
        <div className="text-xs text-slate-400 text-right space-y-0.5 pb-4">
