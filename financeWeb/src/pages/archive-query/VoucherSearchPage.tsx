 ...
 *
 * 数据源：archiveStore.records（已组卷归档的记账凭证），与保管视图同源。
 */
...
const VoucherSearchPage: React.FC = () => {
  ...
  const archivedVouchers = useMemo(
    () => records.filter((r) => r.archiveType === '记账凭证' && r.status === '已组卷' && r.volumeId),
    [records],
  );
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 z-10 bg-slate-100 text-left">
            <tr className="border-b border-slate-200">
              <th className="px-5 py-2.5 text-xs font-semibold text-slate-600">凭证号</th>
              <th className="px-5 py-2.5 text-xs font-semibold text-slate-600">日期</th>
              <th className="px-5 py-2.5 text-xs font-semibold text-slate-600">会计科目</th>
              ...
              <th className="px-5 py-2.5 text-xs font-semibold text-slate-600">制单人</th>
              <th className="px-5 py-2.5 text-xs font-semibold text-slate-600">档案状态</th>
          ...
          <tbody>
            {pageData.map((row) => {
              ...
              return (
                <tr key={row.id} className="border-b border-slate-100 hover:bg-sky-50/50 transition-colors">
                  <td className="px-5 py-3 font-mono text-xs font-bold text-slate-800">{row.voucherNo}</td>
                  <td className="px-5 py-3 text-xs text-slate-500 whitespace-nowrap">{row.year}-{row.month}</td>
                  ...
                  <td className="px-5 py-3 text-xs text-slate-600 max-w-[220px] truncate" title={row.remarks}>{row.remarks}</td>
                  ...
                  <td className="px-5 py-3 text-xs text-slate-600">{row.preparer || '—'}</td>
                  <td className="px-5 py-3"><ArchiveStatusTags record={row} /></td>
