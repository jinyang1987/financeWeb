...
import { downloadArchivePackage, transferArchivePackage } from '../../services/packageService';
...
const ArchivePackagePage: React.FC = () => {
  ...
  // ── T15 服务端信息包操作 ──
  const handleDownloadZip = async (pkg: PackageRecord) => {
    if (!pkg.serverNo) return;
    try {
      await downloadArchivePackage(pkg.serverNo);
      triggerToast(`信息包 ${pkg.serverNo} 已下载（服务端校验包级 SHA-256 一致）`, 'success');
    } catch (e) {
      triggerToast('信息包下载失败：' + (e instanceof Error ? e.message : ''), 'warning');
    }
  };

  const handleTransferPackage = async (pkg: PackageRecord) => {
    if (!pkg.serverNo) return;
    try {
      await transferArchivePackage(pkg.serverNo);
      triggerToast(`信息包 ${pkg.serverNo} 已置为「已发送」，包级摘要随交接单传递`, 'success');
    } catch (e) {
      triggerToast('移交失败：' + (e instanceof Error ? e.message : ''), 'warning');
    }
  };
              ...
              <div className="py-2">
                {[...groupedUnits.entries()].map(([type, units]) => (
                  <div key={type} className="mb-1">
                    {/* 分组标题 */}
                    <div className="flex items-center gap-2 px-4 py-1.5 text-xs font-semibold text-slate-500 bg-slate-50 sticky top-0">
                      {typeIcon(type)}
                    ...
                    {units.map(unit => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* 档号范围 */}
                          <span className="text-xs text-slate-400 font-mono hidden xl:block truncate max-w-[180px]">
              ...
              generatedPackages.map(pkg => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* 操作栏（2026-09-05 T15 真封装落地：下载=服务端真 ZIP（含包级摘要验真）；
                        移交=服务端状态机 created → transferred） */}
                    <div className="flex items-center gap-2 px-4 py-2">
                      <button
                        onClick={() => setShowManifestId(showManifestId === pkg.id ? null : pkg.id)}
                        className="flex items-center gap-1 px-2 py-1 text-xs text-slate-500 hover:text-sky-600 hover:bg-sky-50 rounded transition-colors"
                      >
                      ...
                      {pkg.serverNo && (
                        <button
                          onClick={() => void handleDownloadZip(pkg)}
                          className="flex items-center gap-1 px-2 py-1 text-xs text-sky-700 bg-sky-50 border border-sky-200 rounded hover:bg-sky-100 transition-colors"
                          title={`下载归档信息包 ZIP（包号 ${pkg.serverNo}，服务端下载即验真包级 SHA-256）`}
                        >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 text-xs text-slate-500 hover:text-sky-600 hover:bg-sky-50 rounded transition-colors"
                        title="下载封装说明 XML（DA/T 48；本地预览版，ZIP 内含带文件清单的服务端版）"
                      >
                      ...
                      {pkg.serverNo && (
                        <button
                          onClick={() => void handleTransferPackage(pkg)}
                          className="flex items-center gap-1 px-2 py-1 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded hover:bg-emerald-100 transition-colors"
                          title="移交信息包（服务端状态机：已生成 → 已发送；包级摘要随交接单传递）"
                        >
                      ...
                      <div className="flex-1" />
                      {pkg.serverNo && (
                        <span className="font-mono text-xs text-slate-400" title="服务端包号 / 包级 SHA-256（前 16 位）">
                          {pkg.serverNo} · {pkg.checksum.slice(0, 16)}…
                        </span>
                      )}
                      <button
                        onClick={() => removePackage(pkg.id)}
                        className="flex items-center gap-1 px-1.5 py-1 text-xs text-slate-400 hover:text-red-500 rounded transition-colors"
                        title="移除此封装包"
                      >
                    ...
                    {showManifestId === pkg.id && (
                      <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/80">
                        <div className="text-xs text-slate-400 uppercase mb-1 font-semibold">封装说明 XML</div>
                        <pre className="text-xs text-slate-600 bg-white border border-slate-200 rounded-lg p-3 max-h-[240px] overflow-auto font-mono whitespace-pre-wrap">
                          {pkg.manifestXML}
                        </pre>
                      </div>
                    )}
