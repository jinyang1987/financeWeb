        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 divide-x divide-slate-200/80">
              <th className="px-4 py-3 text-left text-sm font-semibold w-20">类型</th>
              <th className="px-4 py-3 text-left text-sm font-semibold w-24">凭证字号</th>
          ...
          <tbody>
            {detail.items.map((it) => (
              <tr key={it.id} className="border-b border-slate-200/60 last:border-0 divide-x divide-slate-100 hover:bg-sky-50/50 transition-colors">
                ...
                <td className="px-4 py-3 text-right font-mono text-sm text-slate-800">{fmtAmount(it.amount)}</td>
                <td className={`px-4 py-3 text-center text-sm font-medium ${ITEM_STATUS[it.status]?.cls || ''}`}>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 h-fit">
            ...
            <p className="text-xs text-slate-400 mb-3">本次抓取的数据流向（默认值在连接配置中设置）</p>
              ...
              {DEST_OPTIONS.map((d) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-400 mt-0.5 leading-relaxed">{d.desc}</div>
                  </div>
                </label>
              ))}
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 divide-x divide-slate-200/80">
                ...
                <th className="px-4 py-3 text-center text-sm font-semibold w-16">触发</th>
                <th className="px-4 py-3 text-center text-sm font-semibold w-20">状态</th>
                <th className="px-4 py-3 text-right text-sm font-semibold w-16">凭证</th>
                <th className="px-4 py-3 text-right text-sm font-semibold w-16">成功</th>
                <th className="px-4 py-3 text-right text-sm font-semibold w-16">跳过</th>
                <th className="px-4 py-3 text-right text-sm font-semibold w-16">失败</th>
                <th className="px-4 py-3 text-right text-sm font-semibold w-16">报表</th>
                <th className="px-4 py-3 text-left text-sm font-semibold">说明</th>
                <th className="px-4 py-3 text-left text-sm font-semibold w-36">开始时间</th>
                ...
                return (
                  <React.Fragment key={b.id}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-mono text-sm text-slate-400">{b.skip_count}</td>
                      <td className="px-4 py-3 text-right font-mono text-sm text-red-600">{b.fail_count}</td>
                      <td className="px-4 py-3 text-right font-mono text-sm text-slate-600">{b.report_count}</td>
                      <td className="px-4 py-3.5 text-sm text-slate-600 max-w-[240px] truncate" title={b.message || ''}>{b.message || '—'}</td>
                      <td className="px-4 py-3 font-mono text-sm text-slate-600">{fmtTime(b.started_at)}</td>
                      <td className="px-4 py-3 text-center">
